...
def loadData(show_im=True):
    ...
    bg = np.mean(data[:100, :100, :], axis=(0, 1))
    data -= bg
    

    data = resize(data, f)

    data = normalize_per_channel(data, type='img')
...
def loadPSFAndGroundTruth():
    # load for background subtraction
    data = skio.imread(imgname)
    data = np.array(data, dtype='float32')

    psf = skio.imread(psfname)
    psf = np.array(psf, dtype='float32')
    ground_truth = skio.imread(gtname)
    ...
    if len(ground_truth.shape) == 2:
        # pad to 3 channels
        ground_truth = np.repeat(ground_truth[..., None], 3, axis=2)
    # remove alpha channel
    ...
    psf = normalize_per_channel(psf, type='psf')
    ground_truth = normalize_per_channel(ground_truth, type='img')

    return psf, ground_truth
...
if __name__ == "__main__":
    ...
    params = yaml.load(open("/Users/clarahung/repos/extendedfov/ADMM-refactor/admm_tuning_config.yml"), Loader=yaml.SafeLoader)
    for k,v in params.items():
        exec(k + "=v")


    psf, ground_truth = loadPSFAndGroundTruth()

    powers = [10 ** -i for i in range(7)]  # from 1.0 to 1e-6
    for tau in powers:
        for mu1 in powers:
            for mu2 in powers:
                for mu3 in powers:
                    ...
                    final_im = rescale(final_im, 0.5, channel_axis=2, anti_aliasing=True) # downsample to meet 8x
                    # plt.imshow(normalize_per_channel(final_im, type='img'))

                    # compute loss
                    mse = compute_mse(final_im, ground_truth)
                    psnr = compute_PSNR(final_im, ground_truth)
                    plt.imshow(np.clip(final_im / np.max(final_im), 0, 1))
                    plt.title('Iters: {}, PSNR: {}, MSE: {}, tau: {}, mu1 {}, mu2 {}, mu3 {}'.format(iters, psnr, mse, tau, mu1, mu2, mu3))
                    plt.axis('off')
                    plt.savefig(f'{resultpath}/psnr-{psnr}-mse-{mse}-tau-{tau}-mu1-{mu1}-mu2-{mu2}-mu3-{mu3}.png',bbox_inches='tight')
                    plt.clf()

                    # Write the loss and tuning parameters into a CSV file
                    csv_file = f"{resultpath}/tuning_results.csv"
                    ...
                    with open(csv_file, mode='a', newline='') as file:
                        ...
                        if not file_exists:
                            writer.writerow(["PSNR", "MSE", "Tau", "Mu1", "Mu2", "Mu3", "Filename"])
                        writer.writerow([psnr, mse, tau, mu1, mu2, mu3, f'psnr-{psnr}-mse-{mse}-tau-{tau}-mu1-{mu1}-mu2-{mu2}-mu3-{mu3}.png'])
                    

    # saveim = input('Save final image? (y/n) ')
    # if saveim == 'y':
    #     filename = input('Name of file: ')
    #     plt.imshow(normalize_per_channel(final_im, type='x'))
    #     plt.axis('off')
